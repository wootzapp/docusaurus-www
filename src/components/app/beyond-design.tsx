		...
		<section className='bg-brand-base py-24 text-brand-text-primary'>
			<div className='container-custom grid gap-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]'>
				<div className='flex flex-col gap-6'>
					...
					<div className='flex flex-col gap-3 rounded-2xl border border-brand-border bg-brand-surface/60 p-6'>
						<h3 className='font-matter text-xl font-semibold text-brand-text-primary'>Expanding the Reward Space</h3>
						<p className='text-sm text-brand-text-secondary sm:text-base'>
				...
				<div className='flex flex-col gap-4'>
					{rewardAreas.map((area) => (
						<article key={area.title} className='flex flex-col gap-2 rounded-2xl border border-brand-border bg-brand-surface/70 p-6 shadow-lg shadow-brand-base/20'>
							<h4 className='font-matter text-lg font-semibold text-brand-text-primary'>{area.title}</h4>
							<p className='text-sm text-brand-text-secondary sm:text-base'>{area.description}</p>
						</article>
					))}
				</div>
			</div>

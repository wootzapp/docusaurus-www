		...
		<section className='bg-slate-900 py-24 text-white'>
			<div className='container-custom flex flex-col gap-14'>
				<div className='mx-auto max-w-3xl text-center flex flex-col gap-4'>
					<span className='mx-auto inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-1 text-xs uppercase tracking-[0.2em] text-white/60'>
				...
				<div className='grid gap-6 md:grid-cols-2 xl:grid-cols-3'>
					{performanceData.map((item) => (
						<article key={item.head} className='flex h-full flex-col gap-4 rounded-2xl border border-white/10 bg-white/5 p-6'>
							<span className='w-fit rounded-md bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-white/70'>
								{item.tag}
							</span>
							<h3 className='font-matter text-xl font-semibold text-white'>
								{item.head}
							</h3>
							<p className='text-sm text-white/70 sm:text-base'>
								{item.para}
